...
import { getCruiseCatalogue } from './cruise-catalogue';
...
// The catalogue is which routes exist, with their slugs, images and ships. It
// used to be a JSON file someone regenerated by hand, so a newly built offer
// did not exist on the site until a developer exported and redeployed; it is
// now read from D1 per request behind a one-minute memo (cruise-catalogue.ts).
// Every price still comes live from D1 — cruise_offers.cheapest_total_pp for
// cards, cruise_flight_prices (+ retail overlay) for the calendar.
// These objects are shared across requests: never mutate them — clone first.
let transformedFrom: RawCruise[] | null = null;
let transformedCruises: HolidayDetail[] = [];

/**
 * The catalogue as HolidayDetail. transformCruise is pure and the catalogue
 * hands back the same array until it refreshes, so identity decides whether the
 * transform has to run again — once a minute rather than once a request.
 */
async function getCruiseHolidays(db: Database): Promise<HolidayDetail[]> {
  const raw = await getCruiseCatalogue(db);
  if (transformedFrom !== raw) {
    transformedCruises = raw.map(transformCruise);
    transformedFrom = raw;
  }
  return transformedCruises;
}
...
async function getCruisePricingFromDb(db: Database, offerId: number): Promise<HolidayPricing | null> {
  ...
  // shipId → name from the catalogue entry.
  const raw = (await getCruiseCatalogue(db)).find(c => c.id === offerId);
...
export async function getHolidayBySlugFromDb(
  ...
  // Check cruises first
  const cruiseEntry = (await getCruiseHolidays(db)).find(c => c.slug === slug);
  if (cruiseEntry) {
...
export async function getAllListedHolidaysFromDb(db: Database): Promise<HolidayDetail[]> {
  ...
  const cruiseCatalogue = await getCruiseHolidays(db);
  const dbCruiseIds = cruiseCatalogue.map(c => c.id - CRUISE_ID_OFFSET).filter(id => id > 0);
...
export async function getNavCountriesFromDb(
  ...
  for (const c of await getCruiseHolidays(db)) {
    names.add(c.country);
  }
